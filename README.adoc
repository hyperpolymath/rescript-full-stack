...
|**rescript-schema**
|Runtime type validation with excellent ergonomics. _Recommended for all API boundaries._
|✅ Stable
...
|**rescript-postgres**
|Type-safe PostgreSQL client with parameterised queries, connection pooling, and transaction support. ReScript FFI bindings to postgres.js (Porsager).
|🔶 Code Complete
|https://github.com/hyperpolymath/rescript-postgres[GitHub]

|**rescript-http-server**
|Thin wrapper around Deno.serve for type-safe HTTP servers. Standalone alternative to the HTTP in rescript-wasm-runtime.
|🔶 Planned
|_TBD_
|===
...
|**rescript-websocket**
|WebSocket client and server with reconnection, heartbeat, and channels
...
deno run -A npm:create-rescript-fullstack my-app
----
